...
def train(
    ...
    if (
        estimator_name == "dectree" or estimator_name == "randforest"
    ):  ## to get reproducibility, need to see random_state
    ...
    clf = GridSearchCV(
        estimator=estimator, param_grid=param_grid, cv=cv, verbose=1, scoring=scorer
    )
