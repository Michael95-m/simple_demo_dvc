...
def model_evaluate(config_path: Text) -> None:
    ...
    test = pd.read_csv(conf["data_split"]["testset_path"])
    ...
    report_folder = conf["evaluate"]["report_folder"]
    if not os.path.exists(report_folder):
        os.makedirs(report_folder)

    logger.info("Save accuracy report")
    accuracy_report_path = conf["evaluate"]["accuracy_report_path"]
    ...
    confusion_matrix_path = conf["evaluate"]["confusion_matrix_path"]
    target_names = conf["evaluate"]["target_names"]
